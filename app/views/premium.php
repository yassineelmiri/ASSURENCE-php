<?php
require(__DIR__ . "/../controllers/Premium.php");
?>
...
<body>
        ...
        <div class="h-screen flex  bg-gray-200">
            ...
            <div class="flex-1 flex flex-col overflow-hidden ">
                ...
                <div class="flex-1 overflow-auto p-4">
                    <h1 class="text-3xl font-bold mb-8">Ajouter Premium</h1>
                            ...
                            <div class="grid grid-cols-1 gap-5">
                                <div>
                                    ...
                                    <input type="text" name="date" id="date" placeholder="Enter date"
                                        class="mt-1 p-2 border border-gray-300 rounded-md w-full">
                                    <label for="claim_id"
                                        class="block text-sm font-medium text-gray-700">claim_id</label>
                                ...
                                <tr>
                                    ...
                                    <td class="py-2 px-4 border-b flex space-x-2 items-center">
                                        ...
                                        <button
                                            ...
                                            onclick="toggleeditPremium(<?= $premium['id'] ?>)">Edit</button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    ...
                    <form action="" method="post" class="flex items-center space-x-2 "
                        id="editPremium<?= $premium['id'] ?>" style="display: none;">
                        <input type="hidden" name="action" value="editPremium">
                        <input type="hidden" name="id" value="<?= $premium['id'] ?>">
                        <input type="text" name="amount" value="<?= $premium['amount'] ?>"
                            class="p-1 border border-gray-300 rounded-md">
                        <input type="text" name="date" value="<?= $premium['date'] ?>"
                            class="p-1 border border-gray-300 rounded-md">
                        <input type="text" name="claim_id" value="<?= $premium['claim_id'] ?>"
                            class="p-1 border border-gray-300 rounded-md">
                        <!-- Repeat for other input fields -->
                        <button type="submit" name="editPremium"
    ...
    <script>
        function toggleeditPremium(insurerId) {
            const editPremium = document.getElementById(`editPremium${insurerId}`);
            editPremium.style.display = (editPremium.style.display === 'none' || editPremium.style.display === '') ? 'flex' : 'none';
        }
    </script>
</body>
